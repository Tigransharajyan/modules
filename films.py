...
class FilmModule(loader.Module):
    ...
    async def film(self, message):
        ...
        try:
            ...
            parts = []
            parts.append(f"<b>{escape(title)}</b> ({escape(year)})")
            parts.append(f"{'<b>Тип:</b> Сериал' if kind == 'series' else '<b>Тип:</b> Фильм'}")
            if plot and plot not in ("N/A", ""):
                parts.append("")
                parts.append(escape(plot))
            def add_if(v, label):
                if v and v not in ("N/A", "-", "—"):
                    parts.append(f"<b>{label}:</b> {escape(v)}")
            add_if(genre, "Жанр")
            add_if(director, "Режиссёр")
            add_if(writers, "Сценарий")
            add_if(actors, "Актёры")
            if country or language:
                cl = f"{country or '—'} / {language or '—'}"
                add_if(cl, "Страна / Язык")
            add_if(awards, "Награды")
            add_if(runtime, "Длительность")
            add_if(total_seasons if kind == "series" else None, "Сезонов")
            add_if(episodes_total if kind == "series" else None, "Эпизодов (точно)")
            add_if(imdb_rating if imdb_rating and imdb_rating != "N/A" else None, "IMDb")
            ...
            add_if(website, "Сайт")
            if imdb_id:
                parts.append(f"<b>Ссылка:</b> <a href='{imdb_link}'>IMDb</a>")
            parts.append(f"<b>Трейлер:</b> <a href='{trailer_search}'>Поиск на YouTube</a>")
            details = "\n".join(parts)
